...
if( !class_exists( 'Skywin_Transactions' ) ):

class Skywin_Transactions {
	...
	public function __construct( $args = array() ) {
		error_log('Skywin_Transactions::__construct');
	...
	public function add_actions() {
		...
		error_log('Skywin_Transactions::add_actions');
	...
	public function wp_enqueue_scripts( ) {
		...
		wp_enqueue_script( 'skywin-'. $this->name .'-js', plugin_dir_url( WC_Skywin_Hub::PLUGIN_FILE ) . 'assets/js/skywin-'. $this->name .'.js', array('jquery'), null, true );
		
		wp_localize_script( 'skywin-'. $this->name .'-js', 'ajax_list_table_params', array(
	...
	public function wp_enqueue_styles( ) {
		...
		wp_enqueue_style( 'skywin-'. $this->name .'-css', plugin_dir_url( WC_Skywin_Hub::PLUGIN_FILE ) . 'assets/css/skywin-transactions.css' );
	...
	public function add_shortcodes() {
		...
		error_log('Skywin_Transactions::add_shortcodes');
	...
	public function create_page_if_not_exist() {
		error_log('Skywin_Transactions::create_page_if_not_exist');
	...
	public function redirect_myaccount(){
		error_log('Skywin_Transactions::redirect_myaccount');
	...
	public function prepare(){	
		error_log('Skywin_Transactions::prepare');
		
		$this->sw_api = Skywin_API();
	...
	public function ajax_list_table () {
		error_log('Skywin_Transactions::ajax_list_table');
	...
	public function get_items( ){
		error_log('Skywin_Transactions::get_items');
		
		$items = array();
		
		if ( $user = wp_get_current_user() ) {
						
			$items = skywin_api()->get_transactions_by_accountNo( $user->Skywin_AccountNo );
	...
	public function filter_items( $items ) {
		error_log('Skywin_Transactions::filter_items');
		...
		if ( !isset( $_REQUEST['startDate'] ) ) {
			...
			$start = date("Y-m-d", strtotime($first["Regdate"]) );
			$end = date("Y-m-d", strtotime($last["Regdate"]) );
	...
	public function sort_items( $items ) {
		error_log('Skywin_Transactions::sort_items');
		
		$order = isset( $_REQUEST['order'] ) ? $_REQUEST['order'] : 'desc';

		$orderby = isset( $_REQUEST['orderby'] ) ? $_REQUEST['orderby'] : 'LastUpd';
		...
		if ( is_array( $items ) ) {
			usort($items, function ($a, $b) use ($order, $orderby, $sortable) {
				...
				if( $order === 'asc' ) {
					...
					if ($orderby === 'LastUpd' || $orderby === 'Regdate') {
	...
	public function get_columns() {
		error_log('Skywin_Transactions::get_columns');
		
		$columns['LastUpd'] = __('Date', 'woo-skywin-hub');
		$columns['Regdate'] = __('Date', 'woo-skywin-hub');
		$columns['AccountType'] = __('Type', 'woo-skywin-hub');
		$columns['Amount'] = __('Amount', 'woo-skywin-hub');
		$columns['Balance'] = __('Balance', 'woo-skywin-hub');
		$columns['Comment'] = __('Comment', 'woo-skywin-hub');
		$columns['AccountNo'] = __('Account', 'woo-skywin-hub');
		$columns['TransNo'] = __('No', 'woo-skywin-hub');
		$columns['TransType'] = __('Type', 'woo-skywin-hub');
	...
	public function column_Regdate( $item ) {
		return '<span class="value">' . date('Y-m-d', strtotime( $item['Regdate'] )) . '</span>';
	}
	
	public function column_LastUpd( $item ) {
		return '<span class="value" style="white-space:nowrap"> '. date('Y-m-d', strtotime( $item['LastUpd'] )) .'</span>' ;
	}
	
	public function column_Amount( $item ) {
		return '<span class="value">' . wc_price(round( $item['Amount'] )) . '</span>';
	}

	public function column_Balance( $item ) {
		return '<span class="value">' . wc_price(round( $item['Balance'] )) . '</span>';
	}
...
function wswh_transactions() {
	return Skywin_Transactions::instance();
}

$wswh_transactions = wswh_transactions();
